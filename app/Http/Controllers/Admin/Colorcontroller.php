...
class Colorcontroller extends Controller
{
	...
	public function store(Request $request)
	{
		...
		if($count == 0)
		{
			...
			$colors->color = $color;
			$colors->save();
			return redirect('/admin/color/list')->with('message','Successfully Submitted');
		}
		else
		{
			return redirect('/admin/color/add')->with('message','Duplicate Data');
		}
	}
	...
	public function destroy($id)
	{
		...
		if($vehicle_colors > 0)
		{
			return redirect('/admin/color/list')->with('message','This color is used with a vehicle record. So you can not delete it.');
		}
		$colors = DB::table('tbl_colors')->where('id','=',$id)->delete();
		return redirect('/admin/color/list')->with('message','Successfully Deleted');
	}
	...
	public function update($id)
	{
		...
		if($count == 0)
		{
			$color->color = $colors;
			$color->save();
			return redirect('/admin/color/list')->with('message','Successfully Updated');
		}
		else
		{
			return redirect('/admin/color/list/edit/'.$id)->with('message','Duplicate Data');
		}
	}
}
