...
class JobCardcontroller extends Controller
{
	...
	public function add_invoice($id)
	{
		...
		$service_pro = DB::table('tbl_service_pros')->where([['service_id','=',$id],['chargeable','=',1]])->SUM('total_price');
			...
			$othr_charges =  DB::table('tbl_service_pros')->where([['service_id','=',$id],['product_id','=',0]])->SUM('total_price');
	...
	public function insert_gatepass_data(Request $request)
    {
		...
		return redirect('/admin/jobcard/list')->with('message','Successfully Submitted');
    }
	//jobcard list
    ...
    public function index()
	{
        $services = DB::table('tbl_services')->orderBy('service_date','asc')
            ->where([['job_no','like','J%'],['reception_id',auth('reception')->user()->id]])
            ->get()->toArray();


		return view('reception-view.jobcard.list',compact('services'));
	}
    ...
    public function search()
    {
        ...
        $Select_company = DB::table('users')->where('role','=','Company')->get()->toArray();
        $Select_vehicles = DB::table('tbl_vehicles')->get()->toArray();

        return view('reception-view.jobcard.search',compact('Select_customer','all_customer','all_vehicles','Select_vehicles','Select_company','all_company'));
    ...
    public function search2()
    {
        ...
        $all_customer=Input::post('customer');
        $all_vehicles=Input::post('vehicle');
        $all_company = Input::post('company');

        $services=DB::table('tbl_services');
//        $services=DB::table('tbl_services')->get()->toArray();

        if ($all_customer != "all")
        {
            $services = $services->where('customer_id',$all_customer);
        }
        if ($all_company != "all")
        {
            $users = User::where('company_id',$all_company)->get();
            $users_id = [];
            foreach ($users as $user)
            {
                $users_id [] = $user->id;
            }
            $services = $services->where(function ($query) use ($users_id) {
                $query->whereIn('customer_id', $users_id);
            });
        }

        if ($all_vehicles != "all")
        {
            $services = $services->where('vehicle_id',$all_vehicles);
        }
        ...
        return response()->json([
            'view' => view('reception-view.jobcard._table', compact('services'))->render(),
        ]);
    }
	...
	public function addproducts()
	{
		...
		<td>
			<input type="text" name="other_product[]" class="form-control">
		</td>

		<td>
			<input type="text" name="other_price[]" class="form-control" id="oth_price" value="<?php if(!empty($pros)) { echo $product->total_price; }?>" >
		</td>

		<td>
			<span class="trash_product" data-id="<?php echo $ids; ?>"><i class="fa fa-trash fa-2x"></i></span>
		</td>
		</tr>
		<?php
	}
	...
	public function view($id)
	{
		...
		return view('reception-view.jobcard.view',compact('viewid','services','tbl_observation_points','tbl_observation_service','tbl_service_observation_points','vehicale','sales','product','s_id','job','pros','pros2','tbl_checkout_categories','first','vehicalemodel','tbl_points','s_date','color','service_data','tax','logo','obser_id','data'));
	}
	...
	public function oth_pro_delete()
	{
		...
		DB::table('tbl_service_pros')->where('id',$del_oth_pro)->delete();
	}
