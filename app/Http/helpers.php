...
//SMS Integration
if(!function_exists('sendPhoneOTP'))
{
    function sendPhoneOTP(string $number,string $token)
    {
        $param = array(
            'username' => 'ilaundry',
            'password' => 'R5RpBUeDFk',
            'senderid' => 'iLaundry',
            'text' => 'Welcome to Smart Motor App. Your verification code is '.$token,
            'type' => 'text',
        ...
        $url = "https://smartsmsgateway.com/api/api_http.php";
        $ch = curl_init();
...
if(!function_exists('send_push_notification_to_device'))
{
    function send_push_notification_to_device($fcm_token, $data)
    {
        ...
        $key = "AAAAyHBdSkE:APA91bEAvCWr3LDz8_xO8U6wvAztyvrclkU6GHilfqM_edP2fWmOB_1F7fiMRzJqQHSDJKKIz3ALyetc8Kjp2afNZzA4I7K5kvma-x9vIVCxUTKaSn5Ext7X9SNsWI2-w7LoG3QwOijX";
...
if (!function_exists('getCustomerMobile')) {
	function getCustomerMobile($id)
	{
		...
		if(!empty($customer))
		{
			$customer_mobile = $customer->contact_no;
