							...
							<?php $i=1;?>
							 @foreach($vehicaltypes as $vehicaltypess)
								<tr>
									...
									<td>
									   <a href="{!! url ('/admin/vehicletype/list/edit/'.$vehicaltypess->id) !!}"> <button type="button" class="btn btn-round btn-success">{{ trans('app.Edit')}}</button></a>

									  <a url="{!! url('/admin/vehicletype/list/delete/'.$vehicaltypess->id)!!}" class="sa-warning"> <button type="button" class="btn btn-round btn-danger dgr">{{ trans('app.Delete')}}</button></a>
									</td>
								</tr>
								<?php $i++; ?>
								@endforeach
